...
class HDFSManager {
 public:
  ...
  struct InputFormat {
    ...
    InputFormat(const Config& config, int num_threads, int app_thread_id) {
      infmt_ = new LineInputFormat(config.input, num_threads, 0, app_thread_id, config.worker_host, config.hdfs_namenode,
