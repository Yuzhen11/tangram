...
void HdfsLoader::FetchBlock(SArrayBinStream bin) {
  std::string hdfs_namenode;
  int hdfs_namenode_port;
  std::string url,fn;
  size_t offset;
  bin >> hdfs_namenode >> hdfs_namenode_port >> url >> offset >> fn;
  offset_ = offset;
  fn_ = fn;
  Init(hdfs_namenode, hdfs_namenode_port, url);
  Load();
  Message msg;
  ...
  SArrayBinStream data_bin;
  data_bin << hostname_;
  msg.AddData(data_bin.ToSArray());
  sender_->Send(msg);
}

void HdfsLoader::Init(std::string hdfs_namenode, int hdfs_namenode_port, std::string& url) {
...
void HdfsLoader::init_blocksize(hdfsFS fs, const std::string& url) {
  int num_files;
...
void HdfsLoader::Load() {
  boost::string_ref line;
  while (next(line)) {
    partition_vec_.push_back(line.to_string());
  }
}

bool HdfsLoader::next(boost::string_ref& ref) {
  if (buffer_.size() == 0) {
...
size_t HdfsLoader::find_next(boost::string_ref sref, size_t l, char c) {
  size_t r = l;
...
void HdfsLoader::handle_next_block() {
  while (true) {
    if (buffer_.empty())
      return;
...
bool HdfsLoader::fetch_new_block() {
  // fetch a new block
...
int HdfsLoader::read_block(const std::string& fn) {
...
boost::string_ref HdfsLoader::fetch_block(bool is_next = false) {
  int nbytes = 0;

  if (is_next) {
    nbytes = hdfsRead(fs_, file_, data_, hdfs_block_size_);
    if (nbytes == 0)
      return "";
    if (nbytes == -1) {
