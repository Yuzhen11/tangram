...
HDFSBrowser::HDFSBrowser(std::string hdfs_namenode, int port)
  ...
  CHECK(suc) << "Failed to connect to HDFS " << hdfs_namenode_ << ":" << hdfs_namenode_port_;
  LOG(INFO) << "Connect to HDFS, namenode:" << hdfs_namenode_ << " port:" << hdfs_namenode_port_;
}
  ...
  for (int i = 0; i < num_files; ++i) {
    ...
    while (k < file_info[i].mSize) {
      ...
      for (int j = 0; j < blk_loc->numOfNodes; ++j) {
        ...
        BlockInfo b{
            std::string(file_info[i].mName) + '\0',
            k,
            hostname
        };
        rets.push_back(b);
