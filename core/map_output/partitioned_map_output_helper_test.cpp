...
TEST_F(TestPartitionedMapOutputHelper, MergeCombineMultipleMapOutput) {
  ...
  for (auto& map_output : map_outputs) {
    ...
    static_cast<TypedMapOutput<int, int>*>(map_output.get())->SetCombineFunc([](int a, int b) { return a + b; });
  }
  static_cast<TypedMapOutput<int, int>*>(map_outputs[0].get())->Add({{2, 1}, {3, 2}});
  static_cast<TypedMapOutput<int, int>*>(map_outputs[1].get())->Add({{3, 3}, {2, 2}});
  static_cast<TypedMapOutput<int, int>*>(map_outputs[2].get())->Add({{1, 4}, {3, 2}});
