...
struct LoadCheckpoint : public PlanBase {
  LoadCheckpoint(int _plan_id, int _cid, std::string _url)
      : PlanBase(_plan_id), cid(_cid), url(_url) {}

  virtual SpecWrapper GetSpec() override {
    SpecWrapper w;
    w.SetSpec<CheckpointSpec>(plan_id, SpecWrapper::Type::kLoadCheckpoint, 
            cid, url);
    return w;
  }
