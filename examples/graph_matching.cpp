...
struct Matcher {
  ...
  std::map<int, std::map<int, std::map<int, std::pair<char, std::vector<int>>>>>
      result;
  ...
  void UpdateResult(Pattern pattern) { // TODO
    ...
    for (int parentPos = 0; parentPos < patternParents.size(); parentPos++) {
      if (parentPos != 0)
        patternChildStartPos += patternParents[parentPos - 1].outlinks.size();
      for (int pos = 0; pos < patternParents[parentPos].outlinks.size();
           pos++) {
    ...
    for (auto tmp1 : result[matched_round - 1]) {
      ...
      for (auto tmp2 : tmp1.second) {
        ...
        if (childNum[vertexId].size() <
            patternParents[patternPos].outlinks.size()) {
          toRemove.insert(vertexId);
        }
      }
    }
    std::stringstream ss;
    ss << "Round: " << matched_round << ". Vertices in matcher " << id
       << " to delete: ";
    for (int tmp : toRemove)
      ss << tmp << ", ";
    ...
    for (auto tmp1 : result[matched_round]) {
      for (auto tmp2 : tmp1.second) {
        for (auto iter = tmp2.second.second.begin();
             iter != tmp2.second.second.end(); iter++) {
  ...
  std::string DebugString() const {
    ...
    for (auto round : result) {
      ...
      for (auto pos : round.second) {
        ...
        for (auto vertex : pos.second) {
          ...
          for (int id : vertex.second.second)
            ss << id << "; ";
          ss << ")";
        }
        ss << "\n";
    ...
    for (auto sibling : sibling_result) {
      if (sibling.second.size() == 0)
        continue;
      ss << "id: " << sibling.first << " -> ";
      for (int id : sibling.second)
        ss << id << ", ";
      ss << "\n";
    }
    return ss.str();
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < num_graph_parts - 1; ++i) {
    ranges.push_back(third_party::Range(i * num_vertices_per_part,
                                        (i + 1) * num_vertices_per_part));
  }
  ranges.push_back(third_party::Range(
      (num_graph_parts - 1) * num_vertices_per_part, FLAGS_num_vertices));
  ...
  Context::mapjoin(
      ...
      [](const Vertex &vertex,
        ...
        o->Add(vertex.id, std::make_pair(vertex.label, vertex.outlinks));
        for (Vertex outlink : vertex.outlinks) {
          o->Add(outlink.id,
                 std::make_pair(outlink.label, std::vector<Vertex>()));
        }
      },
      ...
      ->SetCombine([](std::pair<char, std::vector<Vertex>> *msg1,
                      std::pair<char, std::vector<Vertex>> msg2) {
        CHECK_EQ(msg1->first, msg2.first);
        for (Vertex &vertex : msg2.second)
          msg1->second.push_back(std::move(vertex));
      })
      ->SetName("graph");
  ...
  Context::mapjoin(
      ...
      [num_matchers](const Vertex &vertex, Output<int, int> *o) {
        ...
        o->Add(matcherId, vertex.id);
      },
      [](Matcher *matcher, int msg) { matcher->seeds.insert(msg); })
      ->SetName("matcher");
  ...
  Context::mappartwithjoin(
      ...
      [pattern, num_matcher_parts, num_graph_parts, graph_key_part_mapper,
       num_matchers](
          TypedPartition<Matcher> *p, TypedCache<Vertex> *typed_cache,
          Output<int, MsgT>* o) {
        ...
        for (int i = 0; i < num_graph_parts; i++) {
          ...
          with_parts[idx] =
              std::dynamic_pointer_cast<IndexedSeqPartition<Vertex>>(part);
        }

        for (auto matcher : *p) {
				  std::pair<int, MsgT> MSG;
          ...
          if (matcher.matched_round == -1) { // first round
            for (int seed : matcher.seeds) {
              ...
              if (root->label != pattern.label) {
                MSG.second.push_back(std::make_tuple(
                    -1, -1, root->id, root->label, -1, std::set<int>()));
              } else {
                MSG.second.push_back(std::make_tuple(
                    0, 0, root->id, root->label, -1, std::set<int>()));
              }
            }
          } else { // except first round
            int next_round_pos_start = 0;
            auto pattern_round = pattern.GetRound(matcher.matched_round);
            for (int round_pos = 0; round_pos < pattern_round.size();
                 round_pos++) {
              auto vertices_matcher =
                  matcher.result[matcher.matched_round][round_pos];
              Pattern vertex_pattern = pattern_round.at(round_pos);
              if (round_pos != 0)
                next_round_pos_start +=
                    pattern_round.at(round_pos - 1).outlinks.size();
              for (auto vertex_matcher : vertices_matcher) {
                auto with_p = with_parts[graph_key_part_mapper->Get(
                    vertex_matcher.first)];
                Vertex *vertex_matcher_vertex =
                    with_p->Find(vertex_matcher.first);
                for (const Vertex &outlink : vertex_matcher_vertex->outlinks) {
                  auto with_p =
                      with_parts[graph_key_part_mapper->Get(outlink.id)];
                  Vertex *vertex_to_add = with_p->Find(outlink.id);

                  for (int pos = 0; pos < vertex_pattern.outlinks.size();
                       pos++) {
                    int next_round_pos = pos + next_round_pos_start;

                    if (vertex_to_add->label ==
                        vertex_pattern.outlinks.at(pos).label) {
                      std::set<int> sibling_ids;
                      if (vertex_pattern.outlinks.at(pos).siblings.size() ==
                          0) {
                      } else if (vertex_pattern.outlinks.at(pos)
                                     .siblings.size() == 1) {
                        char sibling_label =
                            vertex_pattern.outlinks.at(pos).siblings[0].label;
                        for (Vertex &v :
                             vertex_matcher_vertex->outlinks) { // TODO
                          if (v.id == vertex_to_add->id)
                            continue;
                      ...
                      MSG.second.push_back(std::make_tuple(
                          matcher.matched_round + 1, next_round_pos,
                          vertex_to_add->id, vertex_to_add->label,
                          vertex_matcher.first, std::move(sibling_ids)));
                    }
                  }
                }
              }
          ...
          if (!MSG.second.empty()) {
            o->Add(MSG.first, std::move(MSG.second)); // do not send empty msg
          }
        }
        for (int i = 0; i < num_graph_parts; i++) {
      ...
      [iteration,
       pattern](Matcher *matcher,
                std::vector<std::tuple<int, int, int, char, int, std::set<int>>>
                    msgs) {
        if (matcher->matched_round == -1) { // first round
          for (auto msg : msgs) {
            if (std::get<0>(msg) == -1) { // root matching failed
              ...
              CHECK_EQ(std::get<5>(msg).size(), 0);
              matcher
                  ->result[std::get<0>(msg)][std::get<1>(msg)][std::get<2>(msg)]
                  .first = std::get<3>(msg);
            }
          }
        } else { // except first round
          for (auto msg : msgs) {
            ...
            for (auto tmp : std::get<5>(msg))
              matcher->sibling_result[std::get<2>(msg)].insert(tmp); // TODO
            matcher
                ->result[std::get<0>(msg)][std::get<1>(msg)][std::get<2>(msg)]
                .first = std::get<3>(msg);
            matcher
                ->result[std::get<0>(msg)][std::get<1>(msg)][std::get<2>(msg)]
                .second.push_back(std::get<4>(msg));
          }
        }
  ...
  Context::mappartjoin(
      ...
      [](TypedPartition<Matcher> *p,
        ...
        for (Matcher &matcher : *p) {
          std::map<int, std::pair<char, std::vector<int>>> &pos10 =
              matcher.result[1][0];
          std::map<int, std::pair<char, std::vector<int>>> &pos11 =
              matcher.result[1][1];
          std::map<int, std::pair<char, std::vector<int>>> &pos20 =
              matcher.result[2][0];
          std::map<int, std::pair<char, std::vector<int>>> &pos30 =
              matcher.result[3][0];
          ...
          for (auto &v : pos30) {
            ...
            for (int parent_id : v.second.second) {
              ...
              exec->Add([b_id, &result, &sibling_seeds, &pos20, &pos11, &pos10,
                         &matcher, &mu]() {
                ...
                for (int parent_id : pos20[b_id].second) {
                  ...
                  for (int seed : pos11[parent_id].second) {
                    for (int sibling : matcher.sibling_result[parent_id]) {
                      if (sibling == b_id)
                        continue;
                      // if (sibling_seeds[sibling].find(seed) !=
                      // sibling_seeds[sibling].end()) {
                      //  result ++;
                      //}
          ...
          if (result > 0) {
            o->Add(0, std::make_tuple(result, 0, size));
          }
      ...
      ->SetCombine([](std::tuple<int64_t, int64_t, int64_t> *msg1,
                      std::tuple<int64_t, int64_t, int64_t> msg2) {
  ...
  Context::mappartjoin(
      ...
      [](TypedPartition<CountObj> *p, Output<int, int> *o) {
        for (CountObj obj : *p) {
          LOG(INFO) << GREEN("Matched Pattern Number: " +
                             std::to_string(obj.count));
          LOG(INFO) << GREEN(
              "Estimated All Matcher Memory Size: " +
              std::to_string(obj.total_memory) + " = " +
              std::to_string(obj.total_memory / 1024.0 / 1024.0 / 1024.0) +
              "GB");
          LOG(INFO) << GREEN(
              "Estimated Matched Matcher Memory Size: " +
              std::to_string(obj.matched_memory) + " = " +
              std::to_string(obj.matched_memory / 1024.0 / 1024.0 / 1024.0) +
              "GB");
        }
      },
