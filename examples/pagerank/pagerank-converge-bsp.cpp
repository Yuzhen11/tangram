...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 10; ++i) {
    Context::mappartupdate(
        ...
        [](Vertex *v, double contrib) { v->pr += 0.85 * contrib; })
        ->SetCombine([](double *a, double b) { *a = *a + b; }, combine_timeout)
    ...
    Context::write(vertex, write_url,
  ...
  Context::mapupdate(
      ...
      [](TopK *topk, std::vector<std::pair<int, double>> vertices) {
      ...
      ->SetCombine([](std::vector<std::pair<int, double>> *v1,
