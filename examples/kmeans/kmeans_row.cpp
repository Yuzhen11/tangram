...
int main(int argc, char **argv) {
  ...
  auto p2 = Context::mappartjoin(init_points, dense_rows,
    [K, num_dims, num_params, num_param_per_part, num_param_parts](TypedPartition<IndexedPoints>* p,
      AbstractMapProgressTracker* t) {
      std::set<int> indexes; // K index for K clusters (points)
      std::vector<std::pair<int, std::vector<float>>> ret(num_param_parts);
      ...
      if (num_params % num_param_per_part != 0) {
        ret[ret.size()-1].second.resize(num_params % num_param_per_part);
      }
      for (auto& p : all) {
        ret[p.first/num_param_per_part].second[p.first%num_param_per_part] = p.second;
      }
      return ret;
    },
           ...
           num_param_parts, num_param_per_part](TypedPartition<IndexedPoints> *p, TypedCache<DenseRow> *typed_cache,
            ...
            for (auto with_p : with_parts) {
              ...
              while (iter1 != end_iter) {
                ...
                if (row_id == num_param_parts - 1 && num_params % num_param_per_part != 0) {
                  CHECK_EQ(params.size(),  num_params % num_param_per_part);
                } else {
                  CHECK_EQ(params.size(), num_param_per_part);
                }
                std::copy(params.begin(), params.end(), old_params.begin() + row_id*num_param_per_part);
                ++iter1;
              }
            }
