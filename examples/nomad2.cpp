...
int main(int argc, char **argv) {
  ...
  Context::mapupdate(load_collection, data_blocks,
                   ...
                   [](DataBlock *d, Record r) {
                     d->points[r.item].push_back(
                         std::make_tuple(r.user, r.rating, 0));
                   })
  ...
  */
  ...
  Context::mappartwithupdate(
      ...
      [](TypedPartition<DataBlock> *p, TypedCache<Collector> *typed_cache,
          ...
          for (auto &u_r_c : iter->points[item.key]) {
            ...
            float diff =
                -std::inner_product(user.latent, user.latent + kNumLatent,
                                    migrate_item.latent, -std::get<1>(u_r_c));
            /*
            if (c < sample && p->id == 0) {
            ...
            estimated, real: "
                << std::inner_product(user.latent, user.latent+kNumLatent,
            migrate_item.latent, 0.0) << ", " << std::get<1>(u_r_c)
                << ", lr: " << lr << ", t: " << t;
            }
            c += 1;
            */
            ...
            for (int i = 0; i < kNumLatent; ++i) {
              migrate_item.latent[i] +=
                  lr * (user.latent[i] * diff -
                        FLAGS_lambda * migrate_item.latent[i]);
              user.latent[i] += lr * (migrate_item.latent[i] * diff -
                                      FLAGS_lambda * user.latent[i]);
            }
            /*
            diff = - std::inner_product(user.latent, user.latent+kNumLatent,
            migrate_item.latent, - std::get<1>(u_r_c));
            for (int i = 0; i < kNumLatent; ++ i) {
              user_update.latent[i] += lr*(migrate_item.latent[i]*diff -
            FLAGS_lambda*user.latent[i]);
            }
            for (int i = 0; i < kNumLatent; ++ i) {
        ...
        for (auto &msg : msgs) {
          o->Add(msg.first, msg.second);
        }
      },
      [](Collector *collector, Msg msg) {
  ...
  Context::mappartwithupdate(
      ...
      [](TypedPartition<DataBlock> *p, TypedCache<Collector> *typed_cache,
        ...
        static_cast<TypedPartition<Collector>*>(with_part.get())->begin();
          ...
          auto& items = with_iter->items;
          // LOG(INFO) << "[Debug] part id: " << i << ", local item size: " <<
        items.size();
          for (auto& item : items) {
            for (auto& u_r_c: iter->points[item.key]) {
              auto& user = my_with_iter->users[std::get<0>(u_r_c)];
  ...
  Context::foreach (rmse, [](RMSE r) {
    ...
    std::stringstream ss;
    ss << "num samples: " << r.p.first
       << " rmse: " << sqrt(r.p.second / r.p.first) << ", "
